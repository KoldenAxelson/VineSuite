...
| `iot-sensor-integration.md` | **Defer → Phase 8 (with 3 pre-KMP prep items)** | Full IoT layer (LoRaWAN → ChirpStack → MQTT → TimescaleDB) is Phase 8 work across 3 waves (barrel room → fermentation → vineyard). No overlap with KMP shared core. Three lightweight prep items identified for pre-Task-7 execution: (1) swap Docker postgres image to timescale/timescaledb for extension availability, (2) reserve `iot` event source partition in config, (3) reserve 5 IoT event types in enum/config. These are zero-cost, zero-behavioral-change actions that prevent future collisions. Cross-references `water-sgma-tracking.md` (shared soil/flow sensor infra) and Tasks 08, 17, 18, 20. |
| `toast-pos-integration.md` | **Defer → Phase 10 (Task 16 or standalone)** | Bidirectional Toast POS integration for winery-restaurant hybrids. Inbound: Toast order webhooks → normalize to `toast_order_received` events → inventory deduction, lot attribution, gallon conversion for TTB, unified revenue reporting. Outbound: VineSuite inventory changes → Toast Menu API availability updates (auto-86 at zero stock, new vintage availability). Requires menu mapping table linking Toast menu items to VineSuite SKUs/lots with `volume_per_serving` for TTB gallon tracking. Webhook receiver pattern becomes template for future third-party POS integrations (Square, Clover, Lightspeed). Toast has an Integration Partner Program — listing in their marketplace provides discovery to 120K+ restaurants. Cross-references Tasks 09 (POS), 10 (Wine Club), 11 (Ecommerce), 16 (Accounting Integrations), 19 (Reporting), `multi-vertical-cellar-suite.md` (taproom+restaurant is dominant model in craft beer). |
| `public-api-platform-strategy.md` | **Defer → Phase 8 (Task 23)** | Expands Task 23's strategic framing beyond "Pro-tier feature" to "AI-era discovery surface." The Public API is the equivalent of VineBook's SEO flywheel but for AI agents and developer integrations. InnoVint and Commerce7 have no public API — first mover wins the integration ecosystem by default. Key additions to Task 23: OpenAPI 3.1 spec as AI-consumable documentation, Zapier/Make app listing as distribution channel, developer portal with sandbox, read-only API at Basic tier for maximum AI agent connectivity. Event-sourced architecture means webhook catalog is inherently comprehensive (50+ event types = 50+ subscribable webhooks). Cross-references Tasks 23, 24 (VineBook), Toast integration (BYO adapter path), IoT (BYO ingestion), data-portability, and multi-vertical expansion. |
| `multi-vertical-cellar-suite.md` | **Defer → Phase 9+** | Single-codebase multi-vertical expansion via `business_type` tenant field. Wine stays primary; brewery is first adjacent vertical (highest production model overlap, 5130.9 structurally similar to 5120.17). No architectural prep needed — schema-per-tenant, event sourcing, and isolated compliance layer already support it. CellarSuite rebrand avoids vinSuite trademark collision and positions product as craft beverage platform. Do not attempt before wine vertical is revenue-generating with paying customers. |

---

### Pre-Task 7 Triage — Persona-Based Dashboard (2026-03-18)

| Idea | Disposition | Rationale |
|------|-------------|-----------|
| `persona-based-dashboard.md` | **Absorb → Build Pre-Task 7** | Dashboard is a placeholder. Every model and relationship it needs already exists (Phases 2–6). 8 new stock Filament widgets + 5 existing widgets reused. Uses native `getTabs()` for role-based tabs — no custom Tailwind or Blade. Building now means users see value before the codebase shifts to KMP mobile infra where there's no portal-visible output for weeks. All tiers. |
